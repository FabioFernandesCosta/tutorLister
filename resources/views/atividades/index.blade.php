...
<body class="antialiased" id="eventBody">
    <div class="wrapAll">
        ...
        <div id="atvGrid"
            @if (isset($colunas)) style="grid-template-columns: 3.5rem repeat({{ count($colunas) }}{{-- quantidade de colunas --}} , minmax(200px, 1fr));">
        @else
            style="grid-template-columns: 3.5rem repeat({{ 0 }}{{-- quantidade de colunas --}} , minmax(200px, 1fr));"> @endif
            @if (count($atv) > 0) <div> <b> ID </b> </div>
                @if (isset($colunas['descricao']))
                <div> <b> Descrição </b> </div> @endif
            @if (isset($colunas['usuario'])) <div> <b> Usuários envolvidos </b> </div> @endif
            @if (isset($colunas['requisitante'])) <div> <b> Requisitante </b> </div> @endif
            @if (isset($colunas['drealizacao'])) <div> <b> Data de realização </b> </div> @endif
            @if (isset($colunas['hrealizacao'])) <div> <b> Hora de realização </b> </div> @endif
            @if (isset($colunas['dregistro'])) <div> <b> Data do registro </b> </div> @endif
            @if (isset($colunas['hregistro'])) <div> <b> Hora do registro </b> </div> @endif
            @if (isset($colunas['ch'])) <div> <b> Carga horária da atividade </b> </div> @endif
            @if (isset($colunas['status'])) <div> <b> Status </b> </div> @endif
            @foreach ($atv as $key => $value) <div onclick="location.href='{{ URL::to('atividades/' . $value->atividade_id) }}';">
                        {{ $value->atividade_id }}
                    </div>
                    @if (isset($colunas['descricao']))
            ...
            @if (isset($colunas['usuario'])) <div onclick="location.href='{{ URL::to('atividades/' . $value->atividade_id) }}';">
                        {{ $value->nome }}
                    </div> @endif
            @if (isset($colunas['requisitante'])) <div onclick="location.href='{{ URL::to('atividades/' . $value->atividade_id) }}';">
                        {{ $value->requisitante }}
                    </div> @endif
            @if (isset($colunas['drealizacao'])) <div onclick="location.href='{{ URL::to('atividades/' . $value->atividade_id) }}';">
                        {{ $value->data_atividade }}
                    </div> @endif
            @if (isset($colunas['hrealizacao'])) <div onclick="location.href='{{ URL::to('atividades/' . $value->atividade_id) }}';">
                        {{ $value->hora_atividade }}
                    </div> @endif
            @if (isset($colunas['dregistro'])) <div onclick="location.href='{{ URL::to('atividades/' . $value->atividade_id) }}';">
                        {{ $value->data_registro }}
                    </div> @endif
            @if (isset($colunas['hregistro'])) <div onclick="location.href='{{ URL::to('atividades/' . $value->atividade_id) }}';">
                        {{ $value->hora_registro }}
                    </div> @endif
            @if (isset($colunas['ch'])) <div onclick="location.href='{{ URL::to('atividades/' . $value->atividade_id) }}';">
                        {{ $value->carga }}
                    </div> @endif
            @if (isset($colunas['status'])) <div onclick="location.href='{{ URL::to('atividades/' . $value->atividade_id) }}';">
                        {{ $value->status }}
                    </div> @endif
            @endforeach
        @else
